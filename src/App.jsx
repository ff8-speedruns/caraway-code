...
export default function App() {
  ...
  const counts = poles.map((value, position) => (unfinished[position] ? POLE_WILDCARD : value));
  ...
  const setPole = (position, value) =>
    setPoles((current) =>
      current.map((existing, at) => (at === position ? (value ?? '') : existing)),
    );

  const setPoleUnfinished = (position, checked) =>
    setUnfinished((current) =>
      current.map((existing, at) => (at === position ? checked : existing)),
    );
  ...
  const handleDPad = (direction) => {
    ...
    setPoles((current) =>
      current.map((value, position) => {
        if (position !== active) return value;
    ...
    <ToolShell
      ...
      credits={
        <>
          ...
          <Anchor href="https://hyperbolicworld.fr/poteaux/en">Julien Busset</Anchor> and{' '}
          <Anchor href="http://pingval.g1.xrea.com/">Pingval</Anchor>, referencing{' '}
          <Anchor href="https://docs.google.com/document/d/1k0wViIjYPa6oakFvcwXxwmXj_2O_Q8FL9Ab5XRfsA6I">
            Amshagar&apos;s research
          </Anchor>
          .
        </>
    ...
    >
      <Stack gap="lg">
        <SimpleGrid cols={{ base: 3, sm: 6 }}>
          {poles.map((value, position) => (
            <PoleInput
              key={position}
              position={position}
              value={value}
              unfinished={unfinished[position]}
              active={active === position}
              onChange={(next) => setPole(position, next)}
              onUnfinishedChange={(checked) => setPoleUnfinished(position, checked)}
        ...
        {wide && (
          <Alert color="yellow" title="Wide search">
            Searching beyond where a normal run lands. Matches are ordered likeliest first and
            capped at {MAX_VISIBLE_RESULTS} entries.
        ...
        <Group gap="md" justify="center" wrap="wrap">
          ...
          {results.length === 0 && hasCounts && !wide && (
            <Text c="dimmed" ta="center">
              No code matches. If the run did something unusual, the sequence can be valid but fall
              outside this range, so try widening the search below.
            </Text>
          )}

          {results.length === 0 && hasCounts && wide && (
            <Text c="dimmed" ta="center">
              No code matches that sequence anywhere in the searched stream, so at least one count
              is wrong.
            </Text>
          )}
        </Group>
        ...
        <Center>
          <Group gap="md" wrap="wrap" justify="center">
            ...
            <Button
              component="a"
              href={`${import.meta.env.BASE_URL}practice/`}
              rightSection={<IconCaretRightFilled size="1rem" />}
            >
              Practice
            </Button>
          </Group>
        ...
        <Accordion variant="contained">
          <Accordion.Item value="how-to">
            ...
            <Accordion.Panel>
              <Text size="sm" mb="sm">
                Getting into the Caraway Mansion requires the player to provide the guard at the
                entrance with a numerical code, which is normally found at the Tomb of the Unknown
                King. However, the code is determined by the same RNG that controls the sequence of
                poles that pass by the window during the Irvine-Selphie dialogue in the hallway of
                the train to Deling City from Galbadia Garden.
              </Text>
              <List type="ordered" size="sm" spacing="xs">
                <List.Item>
                  Board the train to Deling City after completing the Galbadia Garden story points.
                </List.Item>
                <List.Item>Follow Irvine into the hallway with Selphie.</List.Item>
                <List.Item>
                  Mash through the dialogue while counting the poles that pass by the window (they
                  pass by in groupings of 4-6 sets; an extra-long gap between sets means a set of
                  0).
                </List.Item>
                <List.Item>
                  Once the dialogue is finished, hold down to leave the hallway ASAP.
                </List.Item>
                <List.Item>Note the groups and sequences in the tool.</List.Item>
                <List.Item>
                  If you leave before the last set finishes, tick <Code>?</Code> for it instead of
                  picking a number.
                </List.Item>
                <List.Item>
                  The tool will output the code to give the guard at the mansion.
                </List.Item>
                <List.Item>
                  If there are multiple possible results, you can compare the animations that happen
                  on your way to the mansion with the results shown to narrow it down.
                </List.Item>
              </List>
            </Accordion.Panel>
          </Accordion.Item>
